...
class TestJournalRepository:
    ...
    async def test_get_or_create_today_creates_new(self, repository, mock_session):
        ...
        with patch.object(repository, '_to_model', return_value=DailyJournal()) as mock_to_model: 
    ...
    async def test_get_or_create_today_returns_existing(self, repository, mock_session):
        ...
        mock_db_journal.date = datetime.now(timezone.utc).date()
        
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = mock_db_journal
        mock_session.execute.return_value = mock_result
        
        with patch.object(repository, '_to_model', return_value=DailyJournal(id="existing123")):
            journal = await repository.get_or_create_today()
    ...
    async def test_get_by_date_found(self, repository, mock_session):
        ...
        with patch.object(repository, '_to_model', return_value=DailyJournal(date=target_date)):
    ...
    async def test_get_sessions_by_date(self, repository, mock_session):
        ...
        with patch.object(repository, '_session_to_model', side_effect=lambda s: WorkSession(task=s.task, start_time=datetime.now(timezone.utc))):
    ...
    async def test_get_recent_journals(self, repository, mock_session):
        ...
        with patch.object(repository, '_to_model', side_effect=lambda j: DailyJournal(id=j.id, date=j.date)):
    ...
    def test_session_to_model_conversion(self, repository):
        ...
        mock_db_session.start_time = datetime.now(timezone.utc)
