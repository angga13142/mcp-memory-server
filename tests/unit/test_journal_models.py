...
class TestWorkSession:
    ...
    def test_task_validation_empty(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            WorkSession(task="", start_time=datetime.now(timezone.utc))
...
class TestDailyJournal:
    ...
    def test_get_active_session(self):
        ...
        # Add active session
        session1 = WorkSession(task="Task 1", start_time=datetime.now(timezone.utc))
        ...
        session1.end_time = datetime.now(timezone.utc)
        session2 = WorkSession(task="Task 2", start_time=datetime.now(timezone.utc))
    ...
    def test_total_work_minutes_calculation(self):
        ...
        start1 = datetime.now(timezone.utc) - timedelta(minutes=60)
        ...
        start2 = datetime.now(timezone.utc) - timedelta(minutes=30)
    ...
    def test_tasks_worked_on_count(self):
        ...
        session1 = WorkSession(task="Same Task", start_time=datetime.now(timezone.utc))
        session2 = WorkSession(task="Same Task", start_time=datetime.now(timezone.utc))
        session3 = WorkSession(task="Different Task", start_time=datetime.now(timezone.utc))
...
def sample_journal():
    ...
    journal = DailyJournal(
        morning_intention="Test all features",
        energy_level=4,
        mood="focused"
    )
