...
class DocumentationValidator:
        ...
        for md_file in self.docs_dir.glob('*.md'):
            ...
            for link_text, link_url in links: 
                ...
                if anchor_part: 
                    ...
                    headers = re.findall(r'^#{1,6}\s+(. +)$', target_content, re. MULTILINE)
                    header_anchors = [
    ...
    def _check_completeness(self):
        ...
        required_sections = {
            ...
            'operator-guide.md': ['Installation', 'Configuration', 'Operations', 'Incident Response'],
            'developer-guide.md': ['Quick Start', 'Creating New Metrics', 'Best Practices'],
            'runbook.md':  ['Alert Response', 'Service Recovery', 'Escalation'],
            'troubleshooting. md': ['Quick Diagnostics', 'Common Issues', 'Debugging Tools'],
            'api-reference. md': ['Metrics Reference', 'HTTP Endpoints', 'Alert Rules'],
            'architecture.md':  ['High-Level Architecture', 'Component Architecture', 'Design Decisions'],
        }
        ...
        for filename, sections in required_sections.items():
            ...
            for section in sections:
                ...
                if not re.search(rf'^#{1,6}\s+.*{re.escape(section)}', content, re.MULTILINE | re.IGNORECASE):
                    self. results.append(ValidationResult(
                        passed=False,
