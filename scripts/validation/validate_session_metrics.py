...
async def test_session_metrics():
    ...
    try:
        ...
        match = re.search(
            r'mcp_journal_sessions_total\{status="success"\} (\d+)', baseline
        )
        ...
        result = await journal_service.end_work_session(
            what_i_learned=["Testing metrics"], quick_note="Validation"
        )
        ...
        match = re.search(
            r'mcp_journal_sessions_total\{status="success"\} (\d+)', updated
        )
