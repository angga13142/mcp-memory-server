...
class JournalService:
            ...
            try:
                async with self.database.session() as session:
                    ...
                    if active_session.duration_minutes >= 30:
                        reflection = await self._generate_session_reflection(active_session)
                ...
                logger.info(f"Ended work session: {active_session.task} ({active_session.duration_minutes}min)")
                ...
                return {
                    ...
                    "message": "Session ended. Reflection generated and saved." if reflection_text else "Session ended.",
    ...
    async def _generate_session_reflection(
        ...
        related_results = await self.search_service.search(
            query=session.task,
            limit=3
        )
        ...
        ]) if related_results else "No related past work found."
...
Be concise, insightful, and encouraging."""
    ...
    async def _generate_ai_summary(
        ...
        # Build context
        sessions_summary = "\n".join([
            f"- {s.task} ({s.duration_minutes}min)"
            for s in journal.work_sessions
        ])
        
        learnings_text = "\n".join([f"- {l}" for l in learnings[:5]]) if learnings else "None recorded"
        challenges_text = "\n".join([f"- {c}" for c in challenges[:5]]) if challenges else "None recorded"
