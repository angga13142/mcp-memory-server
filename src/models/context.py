...
class ActiveContext(BaseModel):
    ...
    current_task: str = Field(default="", description="Current task being worked on")
    related_files: list[str] = Field(
        default_factory=list, description="Files relevant to current work"
    )
    relevant_decisions: list[str] = Field(
        default_factory=list, description="IDs of relevant decisions"
    )
    notes: str = Field(default="", description="Free-form notes about current context")
    working_branch: str = Field(default="", description="Git branch being worked on")
