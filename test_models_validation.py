...
async def validate_work_session_model():
    ...
    assert 44 <= session.duration_minutes <= 46, f"Duration should be ~45 min, got {session.duration_minutes}"
...
async def validate_daily_journal_model():
    ...
    assert journal.date == datetime.now(timezone.utc).date(), "Date should default to today"
